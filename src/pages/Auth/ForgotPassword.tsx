...
import { Mail, AlertTriangle, CheckCircle, ArrowLeft, Eye, EyeOff } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  return (
    <div>
      <Link to="/login" className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-700 transition-colors mb-6">
        <ArrowLeft size={14} /> {trans.auth.signInLink}
      </Link>

      <div className="mb-7">
        <h2 className="text-[26px] font-bold text-ink-900 tracking-[-.02em] leading-tight">{trans.auth.forgotPasswordTitle}</h2>
        <p className="text-sm text-ink-400 mt-1.5">{trans.auth.forgotPasswordSubtitle}</p>
      </div>
      ...
      {success && (
        <div className="flex items-center gap-2.5 text-xs text-forest-700 bg-forest-50 border border-forest-200 rounded-lg px-3.5 py-2.5 mb-5 leading-relaxed">
